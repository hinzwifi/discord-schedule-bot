...
<section class="title-bar"> 
        ...
        <aside class="calendar__sidebar"> 
            ...
            <a href="javascript:void(0);" style="width:100%;"class="add-event-btn btn btn-primary">Add event</a> 
            <div id="event_add_frm" style="display:none;"> 
                <form id="eventAddFrm" style="    margin-right: 15%;
    margin-top: 20px;
    margin-left: 15%;" action="#"> 
                    ...
                    <div class="form-group"> 
                        ...
                        <input type="text" class="time form-control" placeholder="From" name="event_time" id="event_time" required/>
                        <input type="text" class="time form-control" placeholder="To"name="event_time" id="event_time" required/>
                    </div> 
                    ...
                    <input type="submit" name="event_submit" class="btn btn-primary" value="Submit"> 
                </form> 
            </div> 
        </aside> 
...
function getEvents($date = ''){ 
    ...
    global $db; 
    $result = $db->query("SELECT title, time FROM events WHERE date = '".$date."' AND status = 1"); 
    if($result->num_rows > 0){ 
        ...
        while($row = $result->fetch_assoc()){ $i++; 
            $eventListHTML .= '<li class="sidebar__list-item"><span class="list-item__time">'.$i.'.</span><span>'.$row['title'].'</span> <span>'.$row['time'].'</span> to </li>'; 
        } 
        $eventListHTML .= '</ul>'; 
    } 
    echo $eventListHTML; 
...
function addEvent($postData){ 
    $status = 0; 
    if(!empty($postData['event_title']) && !empty($postData['event_time']) && !empty($postData['event_date'])){ 
        global $db; 
        $event_title = $db->real_escape_string($postData['event_title']); 
        $event_time = $db->real_escape_string($postData['event_time']); 
       $insert = $db->query("INSERT INTO events (title, time, date, created, modified) VALUES ('".$event_title."', '".$event_time."','".$postData['event_date']."', '".$postData['event_date']."', '".$postData['event_date']."')"); 
        if($insert){ 
            $status = 1; 
        } 
    } 
